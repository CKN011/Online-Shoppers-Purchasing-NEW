...
st.header("🧾 Nutzereingaben")
input_dict = {
    "Administrative_Duration": st.slider("Administrative_Duration", 0.0, 1000.0, 50.0),
    "Informational_Duration": st.slider("Informational_Duration", 0.0, 500.0, 20.0),
    "ProductRelated_Duration": st.slider("ProductRelated_Duration", 0.0, 5000.0, 200.0),
    "BounceRates": st.slider("BounceRates", 0.0, 1.0, 0.2),
    "ExitRates": st.slider("ExitRates", 0.0, 1.0, 0.25),
    "PageValues": st.slider("PageValues", 0.0, 100.0, 5.0),
    "SpecialDay": st.slider("SpecialDay", 0.0, 1.0, 0.0),
    "Month": month_map[st.selectbox("Month", list(month_map.keys()))],
    "VisitorType": visitor_map[st.selectbox("VisitorType", list(visitor_map.keys()))]
}
